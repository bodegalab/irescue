...
def parseArguments():
    parser = argparse.ArgumentParser(
        ...
        description=(
            "IRescue (Interspersed Repeats single-cell quantifier): a tool"
            "for quantifying transposable elements expression in scRNA-seq."
        ),
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=(
            "Genomic TE coordinates in bed format (at least 4 columns with TE"
            " feature name (e.g. subfamily) as the 4th column). "
    ...
    parser.add_argument(
        ...
        help=(
            "Genome assembly symbol. One of: {} (default: "
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=(
            "Text file of filtered cell barcodes by e.g. Cell Ranger, "
            "STARSolo or your gene expression quantifier of choice "
            "(Recommended. default: %(default)s)."
            " Note: If not provided, all barcodes found in BAM will be used."
        ),
    )
    parser.add_argument(
        "-c",
        ...
        metavar="STR",
        help=(
            "BAM tag containing the cell barcode sequence"
            " (default: %(default)s)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="INT",
        help=(
            "Minimum overlap between read and TE as number of nucleotides "
            "(Default: disabled)."
        ),
    )
    parser.add_argument(
        "--min-fraction-overlap",
        type=float,
        metavar="FLOAT",
        choices=[x / 100 for x in range(101)],
        help=(
            "Minimum overlap between read and TE as a fraction of "
            "read's alignment (i.e. 0.00 <= NUM <= 1.00) (Default: disabled)."
        ),
    )
    parser.add_argument(
        "--max-iters",
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to samtools binary, in case it's not in PATH "
            "(Default: %(default)s)."
        ),
    )
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to bedtools binary, in case it's not in PATH "
            "(Default: %(default)s)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Writes additional logging to stderr. "
            "Use once for normal verbosity (-v), twice for debugging (-vv)."
        ),
    )
    parser.add_argument(
        "-V",
